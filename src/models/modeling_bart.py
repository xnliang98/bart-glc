...
def get_sentence_embedding(sentence_mask, hidden_states, sentence_id):
    ...
    # print(mask)
    # if mask.shape[-1] != hidden_states.size()[-1]
...
def get_local_score(cluster_id_lst, embs):
    ...
    # local_scores = F.normalize(local_scores, p=2, dim=0)
    # print(local_scores)
    return local_scores

def get_global_local_centrality_score(sens_emb):
    clusters, dist = kmeans(sens_emb, len(sens_emb), niter=10)
...
class BartEncoder(BartPretrainedModel):
    ...
    def forward(
        ...
        if sentence_mask is None:
            ...
            # compute centrality * doc-sen similarity
            sens = []
            ...
            for sens_emb, sens_mask in zip(sens_embs, sentence_mask):
                # gl_scores, global_scores, local_scores = get_global_local_centrality_score(sens_emb)
                # gl_scores = np.array(gl_scores) / np.linalg.norm(gl_scores)

                gl_scores1, global_scores, local_scores = get_global_local_centrality_score(sens_emb)
                gl_scores2, global_scores, local_scores = get_global_local_centrality_score(sens_emb)
                gl_scores3, global_scores, local_scores = get_global_local_centrality_score(sens_emb)
            ...
            # import glob
            # files = glob.glob("analysis/*.pt")
            # num = max([int(x.split("/")[-1][:-3]) for x in files]) + 1
            # torch.save(input_ids.detach().cpu().numpy(), f"analysis/{str(num)}.pt.i")
            # torch.save(sens_embs.detach().cpu().numpy(), f"analysis/{str(num)}.pt")
            # torch.save(new_weights.detach().cpu().numpy(), f"analysis/{str(num)}.pt.w")

            lam = 0.5
            hidden_states = lam * (hidden_states * new_weights) + (1-lam) * hidden_states
            # hidden_states = (hidden_states * far_weights) + hidden_states

            # matrix = torch.matmul(sens_embs, sens_embs.permute(0, 2, 1))
            

            # far_weights = []
            # for m, sen_mask in zip(matrix, sentence_mask):
            #     # new_m = m - torch.diag_embed(torch.diag(m))
            #     new_m = m
            #     doc = F.normalize(new_m[0, 1:], p=2, dim=0)
            #     # print(doc)
            #     sens = F.normalize(torch.sum(new_m[1:, 1:], 1), p=2, dim=0)
            #     # print(sens)
            #     far_score = F.normalize(doc * sens, p=2, dim=0)
            #     # far_score = sens
            #     tmp = (sen_mask == 0).float() * 0 
            #     for idx in range(1, torch.max(sen_mask) + 1):
            #         tmp += far_score[idx-1] * (sen_mask == idx).float()

            #     far_weights.append(tmp.unsqueeze(-1))
            # far_weights = torch.stack(far_weights).expand(hidden_states.size())


            # lam = 0.5
            # hidden_states = lam * (hidden_states * far_weights) + (1-lam) * hidden_states
            # hidden_states = (hidden_states * far_weights) + hidden_states
...
class CenBartModel(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
    ...
    def forward(
        ...
        # if cen_weight is not None:
        #     cen_weight = cen_weight.unsqueeze(-1).expand([cen_weight.shape[0], cen_weight.shape[1], encoder_outputs[0].shape[-1]])
        #     encoder_outputs[0] = encoder_outputs[0] * cen_weight
        # decoder outputs consists of (dec_features, past_key_value, dec_hidden, dec_attn)
        decoder_outputs = self.decoder(
            input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask,
...
class CenBartForConditionalGeneration(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head.weight"]

    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = CenBartModel(config)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
